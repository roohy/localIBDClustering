...
Modified by Ruhollah Shemirani for compatibility with the latest version of Networkx and added support
...
def HCS(G):
    ...
    if not highly_connected(G, E):
        ...
        if len(sub_graphs) == 2:
            H = HCS(sub_graphs[0])
            _H = HCS(sub_graphs[1])
            G = nx.algorithms.operators.binary.compose(H,_H) #nx.compose is deprecated. 
...
def improved_labelled_HCS(G):
    S = [G.subgraph(c).copy() for c in nx.connected_components(G)] #support for unconnected graphs
    _S = []
    for s in S:
        _S.append(HCS(s))
    _G = nx.algorithms.operators.all.compose_all(_S)
    sub_graphs = [G.subgraph(c).copy() for c in nx.connected_components(_G)]
    max_id = 0 
    labels = None
    
    for _cluster in sub_graphs: #making less assumptions about the number of nodes and their index boundaries. (Some nodes might not show up in clustering results.)
        c = [int(i) for i in list(_cluster.nodes())] #If graph is read from file by networkx, it is prune to have its node numbers stored as string values.
        max_id = max(max(c),max_id)

    labels = np.zeros(shape=(max_id + 1), dtype=np.uint16)
    
    for _class, _cluster in enumerate(sub_graphs, 1):
        c = [int(i) for i in list(_cluster.nodes())]
        labels[c] = _class
